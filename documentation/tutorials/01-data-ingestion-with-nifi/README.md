# Data Ingestion mit Apache NiFi

In diesem Tutorial sehen wir, wie man [Apache NiFi] (http://nifi.apache.org) verwenden kann, um Nachrichten, die von der Better Plattform an Kafka weitergeleitet wurden, und dort als Topic zur Verfügung stehen, automatisch in den S3-Objektspeicher aufgenommen werden können. 

## Prüfen des Buckets in S3

Für diesen Workshop werden wir das Bucket `mdp-demo-test-bucket` verwenden. Wenn sie auf <http://dataplatform:9000> navigieren, und mit dem User `minio` anmelden, dann sollten sie iin der Übersicht das Bucket sehen. 
